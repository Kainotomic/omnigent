...
docker build -f deploy/kainotomic/Dockerfile.host -t omnigent-host:b203ba4c-kt11 .
```
...
```sh
docker run --rm -e OMNIGENT_GATEWAY_API_KEY=dummy omnigent-host:b203ba4c-kt11 \
...
2. Workspaces: project *Kainogent Workspaces*, one raw compose per user from
   ...
   `kainotomic-v0.14.0-kt11`). The `egress` sidecar rejects traffic to `OMNIGENT_EGRESS_DENY_IP`
   ...
   The host image does not ship `kiro-cli`. The entrypoint also deletes any
   leftover `~/.local/bin/kiro-cli` on the persistent home volume (from
   earlier upstream images). The host hello then omits kiro-native, so New
   Chat cannot offer Kiro. Antigravity
