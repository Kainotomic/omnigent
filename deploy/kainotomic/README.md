...
docker build -f deploy/kainotomic/Dockerfile.host -t omnigent-host:b203ba4c-kt8 .
```
...
```sh
docker run --rm -e OMNIGENT_GATEWAY_API_KEY=dummy omnigent-host:b203ba4c-kt8 \
...
2. Workspaces: project *Kainogent Workspaces*, one raw compose per user from
   ...
   `kainotomic-v0.14.0-kt8`). The `egress` sidecar rejects traffic to `OMNIGENT_EGRESS_DENY_IP`
