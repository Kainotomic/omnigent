...
const script = require(path.resolve(".github/kaino/pr-issue-link.js"));
    ...
    graphql: async (query, vars) => {
      ...
      if (query.includes("createdAt")) {
        ...
        return {
          repository: {
            pullRequest: pr
              ? { state: "OPEN", createdAt: "2026-09-15T00:00:00Z", ...pr }
              : null,
          },
        };
      }
...
// The maintainer exemption is removed in this fork: every PR here is authored by
// an org member, so exempting them meant the rule applied to nobody.
for (const assoc of ["MEMBER", "OWNER", "COLLABORATOR"]) {
  assert.strictEqual(
    exemptReason(pr({ number: 30, assoc })),
    null,
    `${assoc} is no longer exempt by association`
  );
}
assert.strictEqual(
  exemptReason(pr({ number: 31, author: "Maintainer-Person", assoc: "CONTRIBUTOR" })),
  null,
  "a listed maintainer is enforced like anyone else"
);
assert.strictEqual(
...
(async () => {
  ...
  {
    const { commented } = await run([pr({ number: 62, draft: true })], {
  ...
  // Role confers no exemption any more: org members and outsiders alike are flagged.
  {
    ...
    assert.deepStrictEqual(
      commented.map((c) => c.issue_number),
      [28, 29, 30],
      "every unlinked PR is commented on regardless of author role"
    );
  }
