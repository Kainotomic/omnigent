...
// every non-draft PR, preferring the owners of the area(s) the PR touches.
//
...
// rotation.
//
// Upstream ran an optional LLM area-fit ranking step before this script, which
// could only REORDER the candidate pool. This fork does not run it (it costs
// gateway spend for a reordering), so selection is pure load-balancing.
//
// Scope: every non-draft PR. Upstream restricted this to fork PRs from
// non-maintainers; in this repo all PRs come from org members on branches, so
// that guard skipped everything. The author is excluded from the candidate pool,
// so nobody is assigned to review their own PR.
//
...
module.exports = async ({ github, context, core }) => {
  ...
  // Fork-only/non-maintainer scoping was upstream's model: outside contributors
  // get a reviewer, maintainers pick their own. Here every PR is authored by an
  // org member on a branch, so that guard would skip all of them. Assignment now
  // runs for every non-draft PR; the author is still excluded from candidates.
