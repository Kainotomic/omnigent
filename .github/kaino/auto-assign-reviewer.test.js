...
process.env.REVIEWER_AREAS_FILE = path.resolve(
  ".github/kaino/auto-assign-reviewer.fixture.json"
);
const script = require(path.resolve(".github/kaino/auto-assign-reviewer.js"));
...
(async () => {
  ...
  // 8. branch (non-fork) PRs are assigned too -- this is the normal case here,
  //    and upstream's fork-only guard skipped every one of them.
  r = await run({ files: ["omnigent/inner/foo.py"], fork: false });
  assert("non-fork PR is assigned", r.added.length === 1, JSON.stringify(r));

  // 9. a maintainer-authored PR is assigned a DIFFERENT maintainer: the author is
  //    excluded from the candidate pool, so nobody reviews their own PR.
  r = await run({ files: ["omnigent/inner/foo.py"], author: "dhruv0811" });
  assert("maintainer-authored PR gets someone other than the author",
    r.added.length === 1 && !r.added.includes("dhruv0811"), JSON.stringify(r));
